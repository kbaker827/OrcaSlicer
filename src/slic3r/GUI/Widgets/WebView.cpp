...
wxWebView* WebView::CreateWebView(wxWindow * parent, wxString const & url)
{
    ...
    if (webView) {
        ...
        // BBS fix #164: WebKitGTK registers URI schemes globally per process; registering
        // the same scheme more than once is a fatal error ("Cannot register URI scheme wxfs
        // more than once"). Guard with a static flag so we only register once per process.
